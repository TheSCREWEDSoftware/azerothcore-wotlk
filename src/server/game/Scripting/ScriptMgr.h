...
public: /* AllCreatureScript */
    ...
    void Creature_SelectLevel(CreatureTemplate const* cinfo, Creature* creature);
...
public: /* Arena Team Script */
    void OnGetSlotByType(uint32 const type, uint8& slot);
    void OnGetArenaPoints(ArenaTeam* at, float& points);
    void OnArenaTypeIDToQueueID(BattlegroundTypeId const bgTypeId, uint8 const arenaType, uint32& queueTypeID);
    void OnArenaQueueIdToArenaType(BattlegroundQueueTypeId const bgQueueTypeId, uint8& ArenaType);
    void OnSetArenaMaxPlayersPerTeam(uint8 const arenaType, uint32& maxPlayerPerTeam);
